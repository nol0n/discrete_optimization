...
namespace obv
{
    ...
    int lpalgs_dev::simplexMethod(Table &table, bool debug)
    {
        ...
        while (true)
        {
            ...
            // найдем элемент в столбце, который будет иметь мин. отношени с первым элементом
            // в первой строке, при этом сам будет отрицательным если все элементы положительные, 
            // то row будет = -1
    ...
    /// @param таблица в столбцовом формате
    /// @return 1 - оптимальный план найден 0 - оптимального плана не сущесвтует
    int lpalgs_dev::dualSimplexMethod(Table &table, bool debug) 
    {
        size_t rows = table.getRows();
        size_t columns = table.getColumns();
        int column = -1;
    ...
    int lpalgs_dev::cuttingPlane(Table &table, bool debug)
    {
    ...
    int lpalgs_dev::integerCuttingPlane(Table &table, bool debug)
    {
        // добавляем строку для записи отсечений
        table.addBottomRow();
